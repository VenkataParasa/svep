...
az acr build --registry $ACR_NAME --image svep-nlp:latest ./nlp_service/

# 3. Build and Push Web Service
echo "🚀 Building and pushing Web App image to Azure..."
az acr build --registry $ACR_NAME --image svep-web:latest ./

# 4. Force Azure Container Apps to pull the latest image and restart
echo "🔄 Updating Container Apps to deploy the new images..."
az containerapp update -n svep-nlp -g $RG --image $ACR_SERVER/svep-nlp:latest
az containerapp update -n svep-web -g $RG --image $ACR_SERVER/svep-web:latest
