...
export async function getLiveIssues(): Promise<LiveIssueCategory[]> {
  ...
  for (const cat of CATEGORIES) {
    ...
    if (apiKey && apiKey.length > 5) {
      try {
        const res = await fetch(`https://v3.openstates.org/bills?jurisdiction=Michigan&q=${cat.query}&sort=updated_desc&per_page=6`, {
        ...
        if (res.ok) {
          ...
          bills = resultsArray.map((b: { id: string; title: string; identifier: string; latest_action_description?: string; latest_action_date?: string; openstates_url?: string; abstracts?: Array<{ abstract?: string; note?: string }> }) => ({
            id: b.id,
            title: b.title,
