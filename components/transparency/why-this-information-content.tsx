...
import { BadgeCheck, FileSearch, MapPin, ScrollText, ShieldQuestion } from "lucide-react";
...
export function WhyThisInformationContent() {
  ...
  const relatedSources = issue
    ...
    : jurisdiction
      ? sources.filter((s) => ["src-detroitmi-council", "src-detroit-opendata-districts", "src-mi-senate-chang", "src-mi-house-directory"].includes(s.id))
      : [];
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="mt-4 flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-semibold tracking-tight">Why am I seeing this information?</h1>
          <p className="text-sm text-muted-foreground">
            Full transparency into how this platform personalizes and sources civic data.
          </p>
        </div>
      </div>
        ...
        <CardContent className="space-y-2 text-sm">
          ...
          <div className="flex justify-between gap-3">
            ...
            <span className="text-right font-medium">
              {jurisdiction ? `${jurisdiction.neighborhood}, ${jurisdiction.city}` : NOT_AVAILABLE}
            </span>
          </div>
          <div className="flex justify-between gap-3">
            <span className="text-muted-foreground">Selected Issue</span>
            <span className="font-medium">{issue ? issue.title : "None selected"}</span>
          </div>
          <div className="flex justify-between gap-3">
            <span className="text-muted-foreground">Government Office Responsible</span>
          ...
          <div className="flex justify-between gap-3">
            ...
            <span className="text-right font-medium">
              {relatedSourcesCount > 0 ? `${relatedSourcesCount} cataloged source${relatedSourcesCount === 1 ? "" : "s"}` : NOT_AVAILABLE}
            </span>
          </div>
          <div className="flex justify-between gap-3">
            <span className="text-muted-foreground">Date Retrieved</span>
            <span className="font-medium">{formatDate(DATA_RETRIEVED_DATE)}</span>
          </div>
          <div className="flex justify-between gap-3">
            <span className="text-muted-foreground">Last Updated</span>
            <span className="font-medium">{lastUpdated ? formatDate(lastUpdated) : NOT_AVAILABLE}</span>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="mt-5 rounded-2xl border-border/80 shadow-sm">
        ...
        <CardContent className="space-y-3">
          {issue && <ConfidenceBadge confidence={issue.confidence} note={issue.demoDataNote} />}
          {jurisdiction && (
          ...
          <p className="text-sm leading-relaxed text-muted-foreground">
            <span className="font-medium text-foreground">Verified</span> means at least one official
            .gov, .senate.gov, or .house.gov source directly confirms this information.{" "}
            <span className="font-medium text-foreground">Demo Data</span> means the record is either a
            reasonable inference (e.g. a ZIP-to-district mapping based on narrative boundary
            descriptions rather than parcel-level GIS data) or illustrative placeholder content, and is
      ...
      <Card className="mt-5 rounded-2xl border-border/80 shadow-sm">
        ...
        <CardContent>
          <p className="text-sm leading-relaxed text-muted-foreground">
            {issue
              ? `This issue was surfaced because it's cataloged as active civic content relevant to Detroit ${jurisdiction ? `and specifically flagged as a top issue for ${jurisdiction.neighborhood}` : "residents"}. Its summary, related legislation, and community impact are derived from the official sources listed below.`
              : jurisdiction
      ...
      <div className="mt-8 flex flex-wrap gap-3 text-sm">
        ...
        <Link href="/audit-trail" className="font-medium text-primary hover:underline">
          View Audit Trail →
        </Link>
      </div>
    </div>
