...
const statusStyles: Record<Candidate["status"], string> = {
  ...
  withdrawn: "border-slate-500/30 bg-slate-500/10 text-slate-600 dark:text-slate-300",
  incumbent: "border-blue-600/30 bg-blue-600/10 text-blue-700 dark:text-blue-400",
};
    ...
    <Link href={`/candidates/${candidate.id}`} className="block h-full">
      <Card className="h-full rounded-2xl border-border/80 shadow-sm transition-shadow hover:shadow-md">
        <CardHeader className="flex-row items-start gap-3">
          <PersonAvatar name={candidate.name} photoUrl={candidate.photoUrl} size="lg" />
          <div className="min-w-0 flex-1">
            <div className="font-semibold">{candidate.name}</div>
            <p className="text-sm text-muted-foreground">{candidate.office}</p>
            <div className="mt-2 flex flex-wrap gap-1.5">
              <PartyBadge party={candidate.party} />
              <Badge variant="outline" className={cn("font-medium", statusStyles[candidate.status])}>
        ...
        <CardContent>
          <p className="line-clamp-3 text-sm text-muted-foreground">{candidate.positionSummary}</p>
          <div className="mt-3">
