...
export function QuickActions({ zip }: { zip: string }) {
  const actions = [
    ...
    { href: `/why-this-information?zip=${zip}`, label: "Why This Information", icon: HelpCircle },
  ];

  return (
    <div className="flex flex-wrap gap-2.5">
      {actions.map((action) => (
        <Button key={action.href} variant="outline" className="gap-2 rounded-xl" render={<Link href={action.href} />}>
